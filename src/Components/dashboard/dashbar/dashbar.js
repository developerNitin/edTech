...
const DashBar = () => {
  ...
  const Schedule = ({ children }) => (
    <div className="m-[32px] mt-[40px] ">{children}</div>
  );
  const DateCell = (props) => {
    return (
      <div
        className={`text-[16px] h-[45px] text-center flex justify-center flex-col items-center after:w-[5px] after:h-[5px] after:rounded-[10px] ${props.className}`}
      >
        <span
          className={`h-[32px] w-[32px] mb-[4px] flex items-center justify-center  cursor-pointer rounded-[30px] ${props.spanClassName}`}
        >
          {props.children}
        </span>
      </div>
        ...
        <div className="grid grid-cols-7">
          ...
          {Array.from({ length: diffDates }).map((_, idx) => (
            <DateCell
              ...
              spanClassName={`hover:bg-black hover:text-white 
              ${
                (idx + 1 === currentDate) &
                (theMonth === currentMonth) &
                (theYear === currentYear)
